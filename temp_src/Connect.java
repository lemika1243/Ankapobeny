...
public class Connect {
    ...
    public static Connection connection(String proper) throws Exception {
        if (proper == null) {
            ...
            properties = proper;
        prop = MiAuto.getAllIn(properties);
    ...
    public static String getDbType() throws Exception {
        ...
        try {
            String[] dbtype = MiAuto.getIn(prop, "dbtype").get(0).split(":");
            valiny = dbtype[1];
        } catch (Exception e) {
    ...
    static String getDbName() throws Exception {
        String valiny = new String();
        try {
            String[] dbtype = MiAuto.getIn(prop, "database").get(0).split(":");
            valiny = dbtype[1];
        } catch (Exception e) {
    ...
    static String getUser() throws Exception {
        String valiny = new String();
        try {
            String[] dbtype = MiAuto.getIn(prop, "user").get(0).split(":");
            valiny = dbtype[1];
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
    ...
    static String getPassword() throws Exception {
        String valiny = new String();
        try {
            String[] dbtype = MiAuto.getIn(prop, "password").get(0).split(":");
            valiny = dbtype[1];
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
